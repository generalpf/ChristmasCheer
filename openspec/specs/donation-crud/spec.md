...
HTTP surface and view layer for listing, searching, filtering, viewing, creating, editing, and deleting donation records, including the nested-under-donor variants of the index and new/create routes. All donation routes are operator-only — authentication comes from `ApplicationController`'s `Authentication` concern.
