...
The design system SHALL be implemented as plain CSS served by Propshaft, organized so that tokens and component styles live in dedicated stylesheet files included via the `application.css` manifest. The change SHALL NOT introduce a new CSS framework, JavaScript styling dependency, or new gem.

#### Scenario: No new dependencies
- **WHEN** the change is implemented
- **THEN** no CSS/JS framework or gem is added, and the styles are delivered through the existing `application.css` asset pipeline
